...
sub grep_FileIDs_to_deface {
    ...
    # base query
    my $query = "SELECT  SessionID, FileID, mst.Name AS ScanType, File "
                . "FROM  files f "
                . "JOIN  mri_scan_type mst ON (f.MriScanTypeID = mst.ID) "
    ...
    if (@modalities_to_deface) {
        @where = map { "mst.Name = ?" } @modalities_to_deface;
    ...
    if (@special_cases) {
        @where  = map { "(mst.Name = ? AND pf.Value LIKE ?)" } @special_cases;
...
sub check_if_deface_files_already_in_db {
    ...
    my @where = map { "mst.Name = ?" } @defaced_scan_types;
...
sub create_defaced_scan_type {
    ...
    my $select     = "SELECT COUNT(*) FROM mri_scan_type WHERE Name = ?";
    my $select_sth = $dbh->prepare($select);
    $select_sth->execute($scan_type);

    my ($count) = $select_sth->fetchrow_array;
    unless ($count) {
        my $insert = "INSERT INTO mri_scan_type SET Name = ?";
