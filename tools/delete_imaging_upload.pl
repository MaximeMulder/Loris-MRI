...
sub getFilesRef {
    ...
    my $mriScanTypeAnd = @$scanTypesToDeleteRef
        ? sprintf(' AND mst.Name IN (%s) ', join(',', ('?') x  @$scanTypesToDeleteRef))
        : '';
...
sub getIntermediaryFilesRef {
    ...
    my $mriScanTypeJoin = @$scanTypesToDeleteRef
        ? 'JOIN mri_scan_type mst ON (f2.MriScanTypeID=mst.ID) '
        : '';
    my $mriScanTypeAnd = @$scanTypesToDeleteRef
        ? sprintf(' AND mst.Name IN (%s) ', join(',', ('?') x @$scanTypesToDeleteRef))
        : '';
...
sub getScanTypesToDelete {
    ...
    my $query = 'SELECT Name, ID FROM mri_scan_type '
           . 'WHERE Name IN ('
           . join(',', ('?') x keys %types)
           . ')';
    my $validTypesRef = $dbh->selectall_hashref($query, 'Name', undef, keys %types);
